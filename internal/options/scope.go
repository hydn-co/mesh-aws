...
type AWSScopeOptionsCore struct {
	Mode                  string          `json:"mode"                              title:"Collection Mode"         description:"Whether the connector collects from a single AWS account or fans out across an AWS Organization."                                                                                 enum:"single,organization"`
	AssumeRoleName        string          `json:"assume_role_name,omitempty"        title:"Assume Role Name"        description:"Name of the cross-account discovery role to STS-assume in each member account (e.g. HyddenDiscoveryRole). Required in organization mode unless static accounts supply role ARNs."`
	ExternalID            string          `json:"external_id,omitempty"             title:"External ID"             description:"Optional sts:ExternalId required by the member-account role's trust policy."`
	IncludeAccountIDs     []string        `json:"include_account_ids,omitempty"     title:"Include Account IDs"     description:"If set, only these member account IDs are collected."`
	ExcludeAccountIDs     []string        `json:"exclude_account_ids,omitempty"     title:"Exclude Account IDs"     description:"Member account IDs to skip during organization collection."`
	OrganizationalUnitIDs []string        `json:"organizational_unit_ids,omitempty" title:"Organizational Unit IDs" description:"If set, limits collection to accounts within these AWS Organizations OUs (and their child OUs)."`
	StaticAccounts        []StaticAccount `json:"static_accounts,omitempty"         title:"Static Accounts"         description:"Explicit member account ID + role ARN entries that bypass AWS Organizations enumeration."`
	Regions               []string        `json:"regions,omitempty"                 title:"Regions"                 description:"Regions to scan for regional services (e.g. Secrets Manager). Defaults to the primary Region when empty."`
	SkipManagementAccount bool            `json:"skip_management_account,omitempty" title:"Skip Management Account" description:"Exclude the management/delegated account from per-account collection."`
}
