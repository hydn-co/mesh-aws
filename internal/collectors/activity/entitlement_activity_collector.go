...
func (c *AWSEntitlementActivityCollector) Start(ctx context.Context) error {
	...
	if err := scope.ForEachTarget(ctx, c.resolver, false, c.newClient,
		func(ctx context.Context, client cloudTrailClient, _ scope.Target) error {
			c.client = client
			return c.collectEntitlementEvents(ctx, startTime, lastEventRef, &emitted)
		}); err != nil {
		return err
	}
...
// collectEntitlementEvents collects and emits entitlement activity for the current target account.
func (c *AWSEntitlementActivityCollector) collectEntitlementEvents(
	ctx context.Context,
	startTime *time.Time,
	lastEventRef string,
	emitted *int,
