...
func (c *AWSAccountActivityCollector) Start(ctx context.Context) error {
	...
	if err := scope.ForEachTarget(ctx, c.resolver, false, c.newClient,
		func(ctx context.Context, client cloudTrailClient, _ scope.Target) error {
			c.client = client
			return c.collectAccountEvents(ctx, startTime, lastEventRef, &emitted)
		}); err != nil {
		return err
	}
...
// collectAccountEvents collects and emits account activity for the current target account.
func (c *AWSAccountActivityCollector) collectAccountEvents(
	ctx context.Context,
	startTime *time.Time,
	lastEventRef string,
	emitted *int,
